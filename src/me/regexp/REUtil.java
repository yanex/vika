...
public class REUtil {
   ...
   *
   * @param expression The expression, beginning with a prefix if it's complex or having no prefix if it's
   *                   simple
  ...
  public static RE createRE(String expression) throws RESyntaxException {
    return REUtil.createRE(expression, RE.MATCH_NORMAL);
  }
  ...
  public static RE createRE(String expression, int matchFlags) throws RESyntaxException {
    if (expression.startsWith(REUtil.complexPrefix)) {
      return new RE(expression.substring(REUtil.complexPrefix.length()), matchFlags);
    }
