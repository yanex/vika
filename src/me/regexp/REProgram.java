...
/**
 * A class that holds compiled regular expressions. You should not need to work directly with this
 * class.
 *
...
public class REProgram {
  ...
  /**
   * Returns a copy of the prefix of current regular expression program in a character array. If
   * there is no prefix, or there is no program compiled yet, <code>getPrefix</code> will return
   * null.
   *
   ...
   * checks for backreferences (so that they can be lazily allocated) and another which attempts to
   * find an prefix anchor string so that substantial amounts of input can potentially be skipped
    ...
    if (instruction != null && lenInstruction != 0) {
      ...
      if (lenInstruction >= RE.nodeSize && instruction[0 + RE.offsetOpcode] == RE.OP_BRANCH) {
        ...
        if (instruction[next + RE.offsetOpcode] == RE.OP_END && lenInstruction >= RE.nodeSize * 2) {
          ...
          // the branch starts with a BOL
            ...
            this.flags |= REProgram.OPT_HASBOL;
          }
        }
      }
      ...
      for (int i = 0; i < lenInstruction; i += RE.nodeSize) {
        switch (instruction[i + RE.offsetOpcode]) {
          ...
          case RE.OP_BACKREF:
            flags |= REProgram.OPT_HASBACKREFS;
