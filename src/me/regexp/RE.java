 ...
 * <p/>
 * <p/>
 * To compile a regular expression (RE), you can simply construct an RE matcher object from the
 ...
 * <p/>
 * <p/>
 * If you were interested in the <i>number</i> of a's which matched the first part of our example
 ...
 * actual regular expression compiler for reasons of efficiency. You can construct a single
 * RECompiler object and re-use it to compile each expression. Similarly, you can change the program
 * run by a given matcher object at any time. However, RE and RECompiler are not threadsafe (for
 * efficiency reasons, and because requiring thread safety in this class is deemed to be a rare
 * requirement), so you will need to construct a separate compiler or matcher object for each thread
 * (unless you do thread synchronization yourself). Once expression compiled into the REProgram
 * object, REProgram can be safely shared across multiple threads and RE objects.
 * <p/>
 * <br>
 * <p/>
 * <br>
 ...
 * <p/>
 * <ul>
 * <li>com.weusours.util.re is not currently compatible with all standard POSIX regcomp flags</li>
 * <li>com.weusours.util.re does not support POSIX equivalence classes ([=foo=] syntax) (I18N/locale
 * issue)</li>
 * <li>com.weusours.util.re does not support nested POSIX character classes (definitely should, but
 ...
 * package, and actually Perl doesn't go to this extent either! Until someone actually complains
 * about this, I'm not sure it's worth "fixing". If it ever is fixed, test #137 in RETest.txt should
 * be updated.</li>
 * </ul>
 * <p/>
 * </font>
...
public class RE {
  ...
  /**
   * *********************************************
   * * The format of a node in a program is: * * [ OPCODE ] [ OPDATA ] [ OPNEXT ] [ OPERAND ] * *
   * char OPCODE - instruction * char OPDATA - modifying data * char OPNEXT - next node (relative
   * offset) * *
   * **********************************************
   */
  ...
  /**
   * Construct a matcher for a pre-compiled regular expression from program (bytecode) data. Permits
   * special flags to be passed in to modify matching behaviour.
   *
  ...
  public boolean match(CharacterIterator search, int i) {
    ...
    if ((program.flags & REProgram.OPT_HASBOL) == REProgram.OPT_HASBOL) {
      ...
      if ((matchFlags & RE.MATCH_MULTILINE) == 0) {
    ...
    if (program.prefix == null) {
      ...
      boolean caseIndependent = (matchFlags & RE.MATCH_CASEINDEPENDENT) != 0;
  ...
  protected boolean matchAt(int i) {
    ...
    int idx;
    if ((idx = matchNodes(0, RE.maxNode, i)) != -1) {
      setParenEnd(0, idx);
      return true;
    }
   ...
   * @param lastNode  Last valid node (used for matching a subexpression without matching the rest of the
   *                  program as well).
  ...
  protected int matchNodes(int firstNode, int lastNode, int idxStart) {
    ...
    for (int node = firstNode; node < lastNode; ) {
      ...
      next = node + (short) instruction[node + RE.offsetNext];
      opdata = instruction[node + RE.offsetOpdata];
      ...
      switch (opcode) {
        ...
        case OP_STAR: {
          ...
          if ((idxNew = matchNodes(node + RE.nodeSize, RE.maxNode, idx)) != -1) {
            return idxNew;
          }
        ...
        case OP_PLUS: {
          ...
          if ((idxNew = matchNodes(next, RE.maxNode, idx)) != -1) {
            return idxNew;
          }
          ...
          node = next + (short) instruction[next + RE.offsetNext];
          continue;
        }
        ...
        case OP_RELUCTANTSTAR: {
          ...
          // matching
          return matchNodes(node + RE.nodeSize, next, idx);
        }

        case OP_RELUCTANTPLUS: {
          // Continue matching the rest without using the reluctant
          // subexpr
          if ((idxNew = matchNodes(next + (short) instruction[next + RE.offsetNext], RE.maxNode,
              idx)) != -1) {
            return idxNew;
          }
        ...
        case OP_BACKREF: {
          ...
          final boolean caseFold = (matchFlags & RE.MATCH_CASEINDEPENDENT) != 0;
        ...
        case OP_BOL:
          ...
          if (idx != 0) {
            ...
            // start of a line
            if ((matchFlags & RE.MATCH_MULTILINE) == RE.MATCH_MULTILINE) {
        ...
        case OP_ESCAPE:
          ...
          switch (opdata) {
            ...
            case E_BOUND: {
              ...
              if (RECharacter.isLetterOrDigit(cLast) == RECharacter.isLetterOrDigit(cNext) == (opdata == RE.E_BOUND)) {
                return -1;
              }
            }
            break;
            ...
            case E_NSPACE:
              ...
              switch (opdata) {
                case E_ALNUM:
                case E_NALNUM:
                  if (!((RECharacter.isLetterOrDigit(c) || c == '_') == (opdata == RE.E_ALNUM))) {
                    return -1;
                  }
                  break;
                ...
                case E_DIGIT:
                case E_NDIGIT:
                  if (!(RECharacter.isDigit(c) == (opdata == RE.E_DIGIT))) {
                    return -1;
                  }
                  break;
                ...
                case E_SPACE:
                case E_NSPACE:
                  if (!(RECharacter.isWhitespace(c) == (opdata == RE.E_SPACE))) {
                    return -1;
                  }
                  break;
              }
        ...
        case OP_ANY:
          ...
          if ((matchFlags & RE.MATCH_SINGLELINE) == RE.MATCH_SINGLELINE) {
            // Match anything
            if (search.isEnd(idx)) {
              return -1;
            }
        ...
        case OP_ATOM: {
          ...
          // int lenAtom = opdata;
          int startAtom = node + RE.nodeSize;
        ...
        case OP_ANYOF: {
          ...
          // casefold
          char c = search.charAt(idx);
          boolean caseFold = (matchFlags & RE.MATCH_CASEINDEPENDENT) != 0;
          // Loop through character class checking our match character
          int idxRange = node + RE.nodeSize;
        ...
        case OP_BRANCH: {
          ...
          if (instruction[next /* + offsetOpcode */] != RE.OP_BRANCH) {
          ...
          do {
            ...
            nextBranch = (short) instruction[node + RE.offsetNext];
            node += nextBranch;
          } while (nextBranch != 0 && instruction[node /* + offsetOpcode */] == RE.OP_BRANCH);
  ...
  public void setProgram(REProgram program) {
    ...
    if (program != null && program.maxParens != -1) {
      ...
      this.maxParen = RE.MAX_PAREN;
    }
  }

  /**
   * Splits a string into an array of strings on regular expression boundaries. This function works
   * the same way as the Perl function of the same name. Given a regular expression of "[ab]+" and a
   * string to split of "xyzzyababbayyzabbbab123", the result would be the array of Strings
   * "[xyzzy, yyz, 123]".
   * <p/>
   * <p/>
   * Please note that the first string in the resulting array may be an empty string. This happens
  ...
  /**
   * Substitutes a string for this regular expression in another string. This method works like the
   * Perl function of the same name. Given a regular expression of "a*b", a String to substituteIn
   * of "aaaabfooaaabgarplyaaabwackyb" and the substitution String "-", the resulting String
   * returned by subst would be "-foo-garply-wacky-".
   *
   ...
   * replaced with the substitution String (if this regular expression object doesn't match
   * at any position, the original String is returned unchanged).
   */
  public String subst(String substituteIn, String substitution) {
    return subst(substituteIn, substitution, RE.REPLACE_ALL);
  }
   ...
   * "&lt;a href=\"$0\"&gt;$0&lt;/a&gt;", the resulting String returned by subst would be
   ...
   * @param flags        One or more bitwise flags from REPLACE_*. If the REPLACE_FIRSTONLY flag bit is set,
   *                     only the first occurrence of this regular expression is replaced. If the bit is not
  ...
  public String subst(String substituteIn, String substitution, int flags) {
    ...
    while (pos < len && match(substituteIn, pos)) {
      ...
      if ((flags & RE.REPLACE_BACKREFERENCES) != 0) {
      ...
      if ((flags & RE.REPLACE_FIRSTONLY) != 0) {
        break;
      }
    }
