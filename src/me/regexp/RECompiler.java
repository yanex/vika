...
public class RECompiler {
  ...
  static {
    RECompiler.hashPOSIX.put("alnum", new Character(RE.POSIX_CLASS_ALNUM));
    RECompiler.hashPOSIX.put("alpha", new Character(RE.POSIX_CLASS_ALPHA));
    RECompiler.hashPOSIX.put("blank", new Character(RE.POSIX_CLASS_BLANK));
    RECompiler.hashPOSIX.put("cntrl", new Character(RE.POSIX_CLASS_CNTRL));
    RECompiler.hashPOSIX.put("digit", new Character(RE.POSIX_CLASS_DIGIT));
    RECompiler.hashPOSIX.put("graph", new Character(RE.POSIX_CLASS_GRAPH));
    RECompiler.hashPOSIX.put("lower", new Character(RE.POSIX_CLASS_LOWER));
    RECompiler.hashPOSIX.put("print", new Character(RE.POSIX_CLASS_PRINT));
    RECompiler.hashPOSIX.put("punct", new Character(RE.POSIX_CLASS_PUNCT));
    RECompiler.hashPOSIX.put("space", new Character(RE.POSIX_CLASS_SPACE));
    RECompiler.hashPOSIX.put("upper", new Character(RE.POSIX_CLASS_UPPER));
    RECompiler.hashPOSIX.put("xdigit", new Character(RE.POSIX_CLASS_XDIGIT));
    RECompiler.hashPOSIX.put("javastart", new Character(RE.POSIX_CLASS_JSTART));
    RECompiler.hashPOSIX.put("javapart", new Character(RE.POSIX_CLASS_JPART));
  }
  ...
  /**
   * Absorb an atomic character string. This method is a little tricky because it can un-include the
   * last character of string if a closure operator follows. This is correct because *+? have higher
   * precedence than concatentation (thus ABC* means AB(C*) and NOT (ABC)*).
   *
    ...
    while (idx < len) {
      ...
      switch (pattern.charAt(idx)) {
...

        {
          ...
          // backreference)
          if ((c & RECompiler.ESC_MASK) == RECompiler.ESC_MASK) {
  ...
  void bracket() throws RESyntaxException {
    ...
    if (pattern.charAt(idx) == '}') {
      idx++;
      bracketOpt = RECompiler.bracketUnbounded;
      return;
    }
  ...
  int branch(int[] flags) throws RESyntaxException {
    ...
    while (idx < len && pattern.charAt(idx) != '|' && pattern.charAt(idx) != ')') {
      // Get new node
      closureFlags[0] = RECompiler.NODE_NORMAL;
      node = closure(closureFlags);
      if (closureFlags[0] == RECompiler.NODE_NORMAL) {
        nullable = false;
      }
    ...
    if (nullable) {
      flags[0] |= RECompiler.NODE_NULLABLE;
    }
  ...
  int characterClass() throws RESyntaxException {
    ...
    if (idx < len && pattern.charAt(idx) == ':') {
      ...
      if (idx + 1 < len && pattern.charAt(idx) == ':' && pattern.charAt(idx + 1) == ']') {
        ...
        Character i = (Character) RECompiler.hashPOSIX.get(charClass);
        if (i != null) {
  ...
  int closure(int[] flags) throws RESyntaxException {
    ...
    int[] terminalFlags = {RECompiler.NODE_NORMAL};
    ...
    switch (closureType) {
      ...
      case '{':
        ...
        if ((terminalFlags[0] & RECompiler.NODE_NULLABLE) != 0) {
    ...
    if (greedy) {
      ...
      switch (closureType) {
        case '{': {
          ...
          if (bracketOpt == RECompiler.bracketUnbounded) {
  ...
  public REProgram compile(String pattern) throws RESyntaxException {
    ...
    int[] flags = {RECompiler.NODE_TOPLEVEL};
  ...
  int expr(int[] flags) throws RESyntaxException {
    ...
    if ((flags[0] & RECompiler.NODE_TOPLEVEL) == 0 && pattern.charAt(idx) == '(') {
    ...
    flags[0] &= ~RECompiler.NODE_TOPLEVEL;
