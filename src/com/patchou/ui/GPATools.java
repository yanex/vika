...
public class GPATools {
  /**
   * Resizes a bitmap with an alpha channel (transparency) without the artifacts introduced by
   * <code>scaleInto()</code>.
   *
   ...
   *                     <code>Bitmap.FILTER_BILINEAR</code> or <code>Bitmap.FILTER_BOX</code>.
   * @param nAspectRatio Specifies how the picture is resized. Can be <code>Bitmap.SCALE_TO_FIT</code>,
   *                     <code>Bitmap.SCALE_TO_FILL</code> or <code>Bitmap.SCALE_STRETCH</code>.
  ...
  public static Bitmap ResizeTransparentBitmap(Bitmap bmpSrc, int nWidth, int nHeight,
    ...
    Graphics graph = null;
    try {
      bmpOrigin = new Bitmap(nOriginWidth, nOriginHeight);
      graph = Graphics.create(bmpOrigin);
    } catch (Exception e) {
      return null;
    }
