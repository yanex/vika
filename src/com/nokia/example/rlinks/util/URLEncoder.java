...
public class URLEncoder {
  ...
  private static String hex(int sym) {
    return URLEncoder.hex.substring(sym * 3, sym * 3 + 3);
  }
  ...
  public static String urlEncode(String s) {
    ...
    for (int i = 0; i < len; i++) {
      ...
      if ('A' <= ch && ch <= 'Z') { // 'A'..'Z'
        ...
        sbuf.append(URLEncoder.hex(ch));
      } else if (ch <= 0x07FF) { // non-ASCII <= 0x7FF
        sbuf.append(URLEncoder.hex(0xc0 | ch >> 6));
        sbuf.append(URLEncoder.hex(0x80 | ch & 0x3F));
      } else { // 0x7FF < ch <= 0xFFFF
        sbuf.append(URLEncoder.hex(0xe0 | ch >> 12));
        sbuf.append(URLEncoder.hex(0x80 | ch >> 6 & 0x3F));
        sbuf.append(URLEncoder.hex(0x80 | ch & 0x3F));
      }
    }
