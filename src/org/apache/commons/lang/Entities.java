...
class Entities {
  ...
  static class HashEntityMap extends MapIntMap {
    ...
    public HashEntityMap() {
      mapNameToValue = new Hashtable();
      mapValueToName = new Hashtable();
    }
  }
  ...
  static abstract class MapIntMap implements Entities.EntityMap {
    protected Hashtable mapNameToValue;
    protected Hashtable mapValueToName;
  ...
  static {
    XML = new Entities();
    Entities.XML.addEntities(Entities.BASIC_ARRAY);
    Entities.XML.addEntities(Entities.APOS_ARRAY);
  }

  static {
    HTML32 = new Entities();
    Entities.HTML32.addEntities(Entities.BASIC_ARRAY);
    Entities.HTML32.addEntities(Entities.ISO8859_1_ARRAY);
  }

  static {
    HTML40 = new Entities();
    Entities.fillWithHtml40Entities(Entities.HTML40);
  }
   ...
   * </p>
   *
   * @param writer The <code>Writer</code> to write the results of the escaping to. Assumed to be a
   *               non-null value.
   ...
   * @see Writer
   */
  public void unescape(StringBuffer writer, String string) throws IOException {
    ...
    for (int i = 0; i < len; i++) {
      ...
      if (c == '&') {
        ...
        if (entityContentLen > 0) {
          if (entityContent.charAt(0) == '#') { // escaped value content
            // is an integer
            // (decimal or
            // hexidecimal)
        ...
        if (entityValue == -1) {
          writer.append('&');
          writer.append(entityContent);
          writer.append(';');
        } else {
          writer.append((char) entityValue);
        }
